 ...
 *
 * Run this after every `import-curated.ts` run — the web app reads words.json
 * directly, so an import alone changes nothing that a user can see.
 *
 *   node apps/api/scripts/build-words.mjs
 */
...
const corpus = read(join(here, '../src/container/demo-corpus.json'));
const curated = read(join(here, '../src/container/curated-mnemonics.json'));
const out = join(here, '../../web/src/data/words.json');
